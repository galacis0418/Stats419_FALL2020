...
hstrvec = c("8' 2.25''","7' 5.25''");

convertNBA = function(hstrvec)
{
  res = c();
  for(hstr in hstrvec)
  {
    hstr = gsub("'","",hstr, fixed=TRUE);
    tmp = strsplit(hstr, " ", fixed=TRUE)[[1]];
    inches = 0;
    
    inches = 12 * as.numeric(tmp[1]);
    if(!is.na(tmp[2])) { inches = inches + as.numeric(tmp [2]); }
    res = c(res,inches);
  }
  res;
}

buildLinearRegressionModel = function(headheight, height)
{
  X <- headheight
  y <- height
  
  plot(X,Y, xlim=range(X), ylim=range(y));
  linear.model = lm(Y~X, measure.df.new);
